...
export default class Queue {
  ...
  constructor(
    queueName: Queues,
    payload?: Omit<bullMQ.QueueOptions, "connection">,
  ) {
  ...
  private setQueueName(name: Queues) {
    ...
    if (!QueuesValues.includes(name)) {
      throw new Error("Queue name doesnt exist on Queues enum!");
    }
    this.queueName = name;
  }
  ...
  func(processor?: unknown) {
    ...
    this.worker.on("completed", (job) => {
      ...
      if (JobsConfiguration.outputConfiguration.outputToJsonFile) {
        ...
        Deno.writeTextFile(
          `jobs/output/${this.queueName}/${job.processedOn}.json`,
